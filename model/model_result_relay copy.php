...
// 신기록선수 : 신기록결과값 가져옴
$get_new_record = "SELECT worldrecord_athletics,MIN(ROUND(cast(worldrecord_record as float),2)) AS worldrecord_record 
    ...
    if ($j == 3) {
        ...
        for ($j = 0; $j < count($worldrecord); $j++) {
            if ($record[$in] < $worldrecord[$j]) {
                $new = 'y';
                $insert_new_record = "insert into list_worldrecord(worldrecord_sports, worldrecord_location, worldrecord_gender,worldrecord_athlete_name,
...
// echo "insert_new_record : ";
// print_r($insert_new_record);
echo "<br>";
echo "savequery : ";
print_r($savequery);
